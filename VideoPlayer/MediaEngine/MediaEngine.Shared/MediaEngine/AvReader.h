...
public:
	...
	using _AvSourceStreamManagerSafeObj = H::ThreadSafeObject<std::recursive_mutex, std::unique_ptr<AvSourceStreamManager>>;
...
#if !ASYNC_READER
    // Test simple reading
    std::unique_ptr<MF::MFVideoSample> ReadNextVideoSample();
#endif
...
private:
    ...
    // Threads
    //
    std::future<void> requestsSamplesRoutine;
    //std::shared_ptr<H::ConcurrentQueue<H::movable_function<void()>>> requestsSamplesQueue;
    std::shared_ptr<H::ConcurrentQueue<std::function<void()>>> requestsSamplesQueue;
};