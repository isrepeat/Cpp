...
#if !ASYNC_READER
	// Must be called in same thread where called Render()
	void VideoSceneRenderer::ReadNeccessaryVideoSamples() {
		if (this->queryNextSample) {
			this->queryNextSample = false;
			auto dxDev = this->swapChainPanel->GetDxDevice()->Lock();
			auto dxCtx = dxDev->LockContext();
			auto d2dCtx = dxCtx->D2D();
			d2dCtx->SaveDrawingState(this->stateBlock.Get());
			this->nextVideoSample = this->avReader->ReadNextVideoSample();
			d2dCtx->Flush();
	...
	void VideoSceneRenderer::Render() {
		...
		if (this->lastRenderedVideoSample) {
			...
			if (shouldPresentNextSampleInThisIteration) {
				...
				this->lastRenderedVideoSample = std::move(this->nextVideoSample);
				this->RequestNextVideoSample();
			}
		}
		else {
			this->lastRenderedVideoSampleTimePoint = std::chrono::high_resolution_clock::now();
...
#if !ASYNC_READER
	void VideoSceneRenderer::RequestNextVideoSample() {
		this->queryNextSample = true;
	}
#endif
