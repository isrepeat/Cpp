...
namespace GHelper.Mode {
    public class ModeControl {
        ...
        //static SettingsForm settings = AsusDevice.settingsForm;
        ...
        //    return status == PowerSupplyStatus.Adequate || status == PowerSupplyStatus.Overload;
        //}
        ...
        public void AutoPerformance(bool powerChanged = false) {
            SetPerformanceMode(0, true);

            //var Plugged = SystemInformation.PowerStatus.PowerLineStatus;
            //var Plugged = this.IsPluggedIn();
        ...
        public void ResetPerformanceMode() {
            ...
            AsusDevice.acpi.DeviceSet(AsusACPI.PerformanceMode, Modes.GetCurrentBase(), "Mode");
        ...
        public void Toast() {
            //AsusDevice.toast.RunToast(Modes.GetCurrentName(), SystemInformation.PowerStatus.PowerLineStatus == PowerLineStatus.Online ? ToastIcon.Charger : ToastIcon.Battery);
        }


        public void RunPerformanceModeCycle() {
            Task.Run(async () => {
                while (true) {
        ...
        public void SetPerformanceMode(int mode = -1, bool notify = false) {
            ...
            Task.Run(async () => {
                ...
                //if (reset) {
                //    AsusDevice.acpi.DeviceSet(AsusACPI.PerformanceMode, (Modes.GetBase(oldMode) != 1) ? AsusACPI.PerformanceTurbo : AsusACPI.PerformanceBalanced, "ModeReset");
                //    await Task.Delay(TimeSpan.FromMilliseconds(1500));
                //}

                int status = AsusDevice.acpi.DeviceSet(AsusACPI.PerformanceMode, AppConfig.IsManualModeRequired() ? AsusACPI.PerformanceManual : Modes.GetBase(mode), "Mode");
                if (status != 1) {
                    // Vivobook fallback
                    //AsusDevice.acpi.SetVivoMode(Modes.GetBase(mode));
                    AsusDevice.acpi.SetVivoMode(1);
                }
            ...
            //if (AppConfig.Is("xgm_fan") && AsusDevice.acpi.IsXGConnected()) {
            //    //XGM.Reset();
            //}
        ...
        private void ModeToggleTimer_Elapsed(object? sender, System.Timers.ElapsedEventArgs e) {
            ...
            Console.WriteLine($"Timed mode: {Modes.GetCurrent()}");
            SetPerformanceMode();
        ...
        public void AutoFans(bool force = false) {
            ...
            if (AppConfig.IsMode("auto_apply") || force) {
                ...
                int cpuResult = AsusDevice.acpi.SetFanCurve(AsusFan.CPU, AppConfig.GetFanConfig(AsusFan.CPU));
                int gpuResult = AsusDevice.acpi.SetFanCurve(AsusFan.GPU, AppConfig.GetFanConfig(AsusFan.GPU));

                if (AppConfig.Is("mid_fan"))
                    AsusDevice.acpi.SetFanCurve(AsusFan.Mid, AppConfig.GetFanConfig(AsusFan.Mid));


                // Alternative way to set fan curve
                if (cpuResult != 1 || gpuResult != 1) {
                    cpuResult = AsusDevice.acpi.SetFanRange(AsusFan.CPU, AppConfig.GetFanConfig(AsusFan.CPU));
                    gpuResult = AsusDevice.acpi.SetFanRange(AsusFan.GPU, AppConfig.GetFanConfig(AsusFan.GPU));

                    // Something went wrong, resetting to default profile
                    if (cpuResult != 1 || gpuResult != 1) {
                        AsusDevice.acpi.DeviceSet(AsusACPI.PerformanceMode, Modes.GetCurrentBase(), "Reset Mode");
                ...
                if ((AppConfig.IsPowerRequired() || xgmFan) && !AppConfig.IsMode("auto_apply_power")) {
                    Task.Run(async () => {
                        await Task.Delay(TimeSpan.FromSeconds(1));
                        AsusDevice.acpi.DeviceSet(AsusACPI.PPT_APUA0, 80, "PowerLimit Fix A0");
                        AsusDevice.acpi.DeviceSet(AsusACPI.PPT_APUA3, 80, "PowerLimit Fix A3");
                    });
                }
        ...
        public void SetPower(bool launchAsAdmin = false) {
            ...
            bool allAMD = AsusDevice.acpi.IsAllAmdPPT();
            ...
            // SPL and SPPT 
            if (AsusDevice.acpi.DeviceGet(AsusACPI.PPT_APUA0) >= 0) {
                AsusDevice.acpi.DeviceSet(AsusACPI.PPT_APUA3, limit_total, "PowerLimit A3");
                AsusDevice.acpi.DeviceSet(AsusACPI.PPT_APUA0, limit_slow, "PowerLimit A0");
            ...
            if (AsusDevice.acpi.IsAllAmdPPT()) // CPU limit all amd models
            {
                AsusDevice.acpi.DeviceSet(AsusACPI.PPT_CPUB0, limit_cpu, "PowerLimit B0");
                customPower = limit_cpu;
            }
            //else if (isAMD && AsusDevice.acpi.DeviceGet(AsusACPI.PPT_APUC1) >= 0) // FPPT boost for non all-amd models
            //{
            //    AsusDevice.acpi.DeviceSet(AsusACPI.PPT_APUC1, limit_fast, "PowerLimit C1");
            //}
        ...
        public void SetGPUClocks(bool launchAsAdmin = true, bool reset = false) {
            ...
            //    if (AsusDevice.acpi.DeviceGet(AsusACPI.GPUEco) == 1) { Logger.WriteLine("Clocks: Eco"); return; }
        ...
        public void SetGPUPower() {
            ...
            if (gpu_power >= AsusACPI.MinGPUPower && gpu_power <= AsusACPI.MaxGPUPower && AsusDevice.acpi.DeviceGet(AsusACPI.GPU_POWER) >= 0)
                AsusDevice.acpi.DeviceSet(AsusACPI.GPU_POWER, gpu_power, "PowerLimit TGP (GPU VAR)");

            if (gpu_boost >= AsusACPI.MinGPUBoost && gpu_boost <= AsusACPI.MaxGPUBoost && AsusDevice.acpi.DeviceGet(AsusACPI.PPT_GPUC0) >= 0)
                boostResult = AsusDevice.acpi.DeviceSet(AsusACPI.PPT_GPUC0, gpu_boost, "PowerLimit C0 (GPU BOOST)");

            if (gpu_temp >= AsusACPI.MinGPUTemp && gpu_temp <= AsusACPI.MaxGPUTemp && AsusDevice.acpi.DeviceGet(AsusACPI.PPT_GPUC2) >= 0)
                AsusDevice.acpi.DeviceSet(AsusACPI.PPT_GPUC2, gpu_temp, "PowerLimit C2 (GPU TEMP)");

            // Fallback
            if (boostResult == 0)
                AsusDevice.acpi.DeviceSet(AsusACPI.PPT_GPUC0, gpu_boost, "PowerLimit C0");
