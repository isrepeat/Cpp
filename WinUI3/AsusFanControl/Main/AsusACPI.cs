...
public class AsusACPI
{
    ...
    public void RunListener()
    {
        ...
        Console.WriteLine("ACPI :" + BitConverter.ToString(data) + "|" + BitConverter.ToString(outBuffer));
        ...
        while (true)
        {
            ...
            Console.WriteLine("ACPI Code: " + code);
        }
    }
    ...
    public AsusACPI()
    {
        ...
        {
            Console.WriteLine($"Can't connect to ACPI: {ex.Message}");
        }
    ...
    public int DeviceSet(uint DeviceID, int Status, string? logName)
    {
        ...
        if (logName is not null)
            Console.WriteLine(logName + " = " + Status + " : " + (result == 1 ? "OK" : result));
    ...
    public int DeviceSet(uint DeviceID, byte[] Params, string? logName)
    {
        ...
        if (logName is not null)
            Console.WriteLine(logName + " = " + BitConverter.ToString(Params) + " : " + (result == 1 ? "OK" : result));
    ...
    public int SetVivoMode(int mode)
    {
        ...
        return AsusDevice.acpi.DeviceSet(VivoBookMode, mode, "VivoMode");
    }
    ...
    public int GetFan(AsusFan device)
    {
        ...
        {
            case AsusFan.GPU:
                fan = AsusDevice.acpi.DeviceGet(GPU_Fan);
                break;
            case AsusFan.Mid:
                fan = AsusDevice.acpi.DeviceGet(Mid_Fan);
                break;
            default:
                fan = AsusDevice.acpi.DeviceGet(CPU_Fan);
                break;
        }
    ...
    public int SetFanCurve(AsusFan device, byte[] curve)
    {
        ...
        if (fanScale != 100 && device == AsusFan.CPU) Console.WriteLine("Custom fan scale: " + fanScale);

        if (AppConfig.IsSwappedFans())
        {
            device = (device == AsusFan.CPU) ? AsusFan.GPU : AsusFan.CPU;
            Console.WriteLine("Swapped fan fix");
        }
    ...
    public byte[] GetFanCurve(AsusFan device, int mode = 0)
    {
        ...
        //Console.WriteLine($"GetFan {device} :" + BitConverter.ToString(result));
    ...
    {
        return (!IsAllAmdPPT() && AsusDevice.acpi.DeviceGet(GPUEco) >= 0 && !AppConfig.IsAlly());
    }
    ...
    public void SetAPUMem(int memory = 4)
    {
        ...
        AsusDevice.acpi.DeviceSet(APU_MEM, mem, "APU Mem");
    }

    public int GetAPUMem()
    {
        int memory = AsusDevice.acpi.DeviceGet(APU_MEM);
    ...
    {
        int value = AsusDevice.acpi.DeviceGet(max ? CORES_MAX : CORES_CPU);
        //value = max ? 0x406 : 0x605;

        if (value < 0) return (-1, -1);
        Console.WriteLine("Cores" + (max ? "Max" : "") + ": 0x" + value.ToString("X4"));
    ...
    public void SetCores(int eCores, int pCores)
    {
        ...
        {
            Console.WriteLine($"Incorrect Core config ({eCores}, {pCores})");
            return;
        };

        int value = (eCores << 8) | pCores;
        AsusDevice.acpi.DeviceSet(CORES_CPU, value, "Cores (0x" + value.ToString("X4") + ")");
    }
    ...
    public void SubscribeToEvents(Action<object, EventArrivedEventArgs> EventHandler)
    {
        ...
        {
            Console.WriteLine("Can't connect to ASUS WMI events");
        }
    }
