...
public static class AppConfig
{
    ...
    static AppConfig()
    {
        ...
        string appPath = Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) + "\\AsusFanControl";
        string configName = "\\config.json";

        //if (System.IO.File.Exists(startupPath + configName))
        //{
        ...
        if (System.IO.File.Exists(configFile))
        {
            ...
            {
                Console.WriteLine($"Broken config: {ex.Message} {text}");
                try
                {
                    text = System.IO.File.ReadAllText(configFile + ".bak");
                    config = JsonSerializer.Deserialize<Dictionary<string, object>>(text);
                }
                catch (Exception exb)
                {
                    Console.WriteLine($"Broken backup config: {exb.Message} {text}");
                    System.IO.File.Copy(configFile, configFile + ".old", true);
                    System.IO.File.Copy(configFile + ".bak", configFile + ".bak.old", true);
                    Init();
                }
            }
        }
    ...
    private static void Timer_Elapsed(object? sender, System.Timers.ElapsedEventArgs e)
    {
        ...
        try
        {
            System.IO.File.WriteAllText(backup, jsonString);
        }
        catch (Exception)
        {
            Thread.Sleep(100);
        ...
        var backupText = System.IO.File.ReadAllText(backup);

        if (backupText.Contains("{") && backupText.Contains("}"))
        {
            System.IO.File.Copy(backup, configFile, true);
        }
        else
        {
            Console.WriteLine("Error writing config");
        }
