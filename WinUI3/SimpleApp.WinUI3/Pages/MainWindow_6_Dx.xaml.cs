namespace SimpleApp.WinUI3 {
    ...
    public sealed partial class MainWindow_6_Dx : Microsoft.UI.Xaml.Window {
        ...
        public byte[] GenerateBGRABytes(int width, int height) {
            byte[] bgraData = new byte[width * height * 4];
            int squareSize = Math.Min(width, height) / 2; // Размер красного квадрата
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    if (x >= startX && x < startX + squareSize && y >= startY && y < startY + squareSize) {
                        bgraData[index] = 0;       // B
                        bgraData[index + 1] = 0;   // G
                        bgraData[index + 2] = 255; // R (красный)
                        bgraData[index + 3] = 255; // A
                    }
                    else {
                        bgraData[index] = 128;     // B
                        bgraData[index + 1] = 128; // G
                        bgraData[index + 2] = 128; // R (серый фон)
                        bgraData[index + 3] = 255; // A
                    }
                }
            }
        ...
        public byte[] GenerateNV12Bytes(int width, int height) {
        ...
        public byte[] ConvertNV12ToBGRA_CPU(byte[] nv12Data, int width, int height) {
            int ySize = width * height;
        ...
        public SharpDX.Direct3D11.Texture2D ConvertNV12TextureToBGRATexture(SharpDX.Direct3D11.Texture2D nv12Texture) {
            ...
            using (var stagingTexture = new SharpDX.Direct3D11.Texture2D(this.d3dDevice, stagingDesc)) {
                this.d3dDevice.ImmediateContext.CopyResource(nv12Texture, stagingTexture);

                var dataBox = this.d3dDevice.ImmediateContext.MapSubresource(stagingTexture, 0,
                ...
                IntPtr uvPtr = dataBox.DataPointer + rowPitch * height;
                for (int y = 0; y < height / 2; y++) {
                    System.Runtime.InteropServices.Marshal.Copy(
                        uvPtr + y * rowPitch,
                        nv12Data,
                ...
                this.d3dDevice.ImmediateContext.UnmapSubresource(stagingTexture, 0);
            }

            byte[] bgraData = ConvertNV12ToBGRA_CPU(nv12Data, width, height);

            // Создаем BGRA-текстуру
            return MakeTextureFromBytes(bgraData, width, height, SharpDX.DXGI.Format.B8G8R8A8_UNorm);
        }

        public SharpDX.Direct3D11.Texture2D MakeTextureFromBytes(byte[] byteArray, int width, int height, SharpDX.DXGI.Format format) {
            ...
            return new SharpDX.Direct3D11.Texture2D(this.d3dDevice, desc, dataRectangles);
        }



        private void Render() {
            var nv12Bytes = this.GenerateNV12Bytes(this.targetWidth, this.targetHeight);
            var nv12Texture = this.MakeTextureFromBytes(nv12Bytes, this.targetWidth, this.targetHeight, SharpDX.DXGI.Format.NV12);
            ...
            var bgraTexture = ConvertNV12TextureToBGRATexture(nv12Texture);
            this.SaveTextureToFile(bgraTexture, "outputTextureCPU");
            ...
            var srvY = new ShaderResourceView(this.d3dDevice, nv12Texture, new ShaderResourceViewDescription {
                Format = SharpDX.DXGI.Format.R8_UNorm, // Y - 8 бит на пиксель
                Dimension = ShaderResourceViewDimension.Texture2D,
                Texture2D = { MipLevels = 1 }
            });

            var srvUV = new ShaderResourceView(this.d3dDevice, nv12Texture, new ShaderResourceViewDescription {
            ...
            this.SaveTextureToFile(this.renderTexture, "outputTextureGPU");
        ...
        private void SaveTextureToFile(SharpDX.Direct3D11.Texture2D texture, string name) {
            ...
            using (var encoder = new SharpDX.WIC.PngBitmapEncoder(factory)) {
                ...
                using (var frame = new SharpDX.WIC.BitmapFrameEncode(encoder)) {
                    ...
                    using (var stagingTexture = new SharpDX.Direct3D11.Texture2D(this.d3dDevice, new SharpDX.Direct3D11.Texture2DDescription {
                        ...
                        this.d3dDevice.ImmediateContext.CopyResource(texture, stagingTexture);

                        var dataBox = this.d3dDevice.ImmediateContext.MapSubresource(stagingTexture, 0, SharpDX.Direct3D11.MapMode.Read, SharpDX.Direct3D11.MapFlags.None);
        ...
        private void SaveBytesToFile(byte[] byteArray, int width, int height, SharpDX.DXGI.Format format, string name) {
            string filename = Windows.Storage.ApplicationData.Current.LocalFolder.Path + $"\\{name}.png";
        ...
        public void SaveTextureToImage(byte[] bgraData, int width, int height, string name, System.Drawing.Imaging.ImageFormat format) {
