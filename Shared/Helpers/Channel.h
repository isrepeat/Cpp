...
public:
    ...
    void Write(std::vector<T>&& writeData, EnumMsg type) {
        ...
        //std::vector<T> msgData = std::move(writeData);
        //msgData.push_back(static_cast<T>(type));
...
template<typename T = uint8_t>
std::vector<T> ReadFileAsync(HANDLE hFile, const std::atomic<bool>& stop) {
    //LOG_DEBUG_D("ReadFileAsync() start");
    enum StatusIO {
        Error,
        Stopped,
        Completed,
    ...
    DWORD dwBytesRead = 0;
    OVERLAPPED stOverlapped = { 0 };
    std::vector<T> tmpBuffer(1024 * 1024 * 50); // NOTE: for simplicity we can use max buffer size ... 2 MB
    ...
    if (ReadFile(hFile, tmpBuffer.data(), tmpBuffer.size() * sizeof(T), &dwBytesRead, &stOverlapped)) {
        ...
        case ERROR_IO_PENDING:
            while (true) {
                ...
                Sleep(10); // TODO: for capturing frame use less delay (CHECK)
            }
            break;
...
void WriteToPipe(HANDLE hNamedPipe, const std::vector<T>& writeData) {
    ...
    do {
        ...
        completed = cbWritten == writeData.size() * sizeof(T);
        if (!completed) {
            int xx = 9;
        }
...
template <typename T = uint8_t>
void WriteToPipe2(HANDLE hNamedPipe, T* data, int size) {
    DWORD cbWritten;
    if (!WriteFile(hNamedPipe, data, size * sizeof(T), &cbWritten, NULL)) { // Write synchronously
        throw PipeError::WriteError;
    }

    //if (cbWritten < writeData.size() * sizeof(T)) {
    //    int xx = 9;
    //}
}
...
public:
    ...
    //    try {
    //        //WriteToPipe<T>(hNamedPipe, writeData);
    //        WriteToPipe<T>(hNamedPipe, std::move(writeData));
    //        if (waitedMessage != EnumMsg::None && waitedMessage == type) {
    //            cvFinishSendingMessage.notify_all();
    //        }
    //    }
    ...
    // Write synchronously
    void Write(const std::vector<T>& writeData, EnumMsg type) {
        //std::unique_lock lk{ mxWrite };
        //writeData.push_back(static_cast<T>(type));
        //WriteToPipe<T>(hNamedPipe, std::move(writeData));


        //std::vector<T> writeData(writeDataTmp);
        std::unique_lock lk{ mxWrite };
        WriteToPipe<T>(hNamedPipe, writeData);
        //writeData.push_back(static_cast<T>(type));
        //if (!connected) {
    ...
    // Write synchronously
    void Write2(T* data, int size, EnumMsg type) {
        std::unique_lock lk{ mxWrite };
        WriteToPipe2<T>(hNamedPipe, data, size);
    }
