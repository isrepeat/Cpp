...
namespace DxDesktop {
	class DxRenderObjProxy : public DxRenderObjProvider<DxRenderObj> {
	public:
		DxRenderObjProxy(Microsoft::WRL::ComPtr<H::Dx::ISwapChainPanel> swapChainPanel)
	...
	private:
		std::unique_ptr<DxRenderObj> CreateObject(H::Dx::DxDeviceSafeObj* dxDeviceSafeObj) {
			...
			auto pixelShaderBlob = Tools::ReadFile(H::ExePath() / L"proxyPS.cso");
