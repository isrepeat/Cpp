...
void TestTaskChainCompletion() {
    // NOTE: next task may executed eather previous task thread or some else (it is undefined)
...
#define TestTaskChainPassedResult_Sequential_Addition

void TestTaskChainPassedResult() {
    LOG_FUNCTION_ENTER("TestTaskChainPassedResult()");
    // NOTE: next task may executed eather previous task thread or some else (it is undefined)

    auto taskA = task<void>([]() {
        LOG_DEBUG_D(L"Task A: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task A: completed");
        //return 1;
        });

#ifdef TestTaskChainPassedResult_Sequential_Addition
    auto taskB = taskA.then([](/*int result*/) {
        LOG_DEBUG_D(L"Task B: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task B: completed");
        //return 2;
        });

    auto taskC = taskB.then([]() {
        LOG_DEBUG_D(L"Task C: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task C: completed");
        return 3;
        });
#else
    taskA = taskA.then([]() {
        LOG_DEBUG_D(L"Task A2: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task A2: completed");
        });

    taskA = taskA.then([]() {
        LOG_DEBUG_D(L"Task A3: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task A3: completed");
        });
#endif

#ifdef TestTaskChainPassedResult_Sequential_Addition
    auto taskD = taskC.then([](int result) {
#else
    auto taskD = taskA.then([]() {
#endif
        LOG_DEBUG_D(L"Task D: started [res = {}]", result);
        Sleep(1'000);
        LOG_DEBUG_D(L"Task D: completed");
        return 4;
        });

    auto taskE = taskD.then([](int result) {
        LOG_DEBUG_D(L"Task E: started [res = {}]", result);
        Sleep(1'000);
        LOG_DEBUG_D(L"Task E: completed");
        });

    auto taskF = taskE.then([]() {
        LOG_DEBUG_D(L"Task F: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task F: completed");
        });

    auto taskG = taskF.then([]() {
        LOG_DEBUG_D(L"Task G: started");
        Sleep(1'000);
        LOG_DEBUG_D(L"Task G: completed");
        });
...
void main() {
    lg::DefaultLoggers::Init(L"ppltask.log");

    //TestTaskChainCompletion();
    TestTaskChainPassedResult();
