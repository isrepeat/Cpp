...
struct fixed_string {
    ...
    char data[N];
    const char constData[2] = "A";
    size_t size;
};

template <size_t N1, size_t N2>
constexpr auto concat(const char(&str1)[N1], const char(&str2)[N2]) {
    fixed_string<(N1 - 1) + N2> result = {}; // skip null-terminator first string

    int idx = 0;

    for (int i = 0; i < N1 - 1; ++i) {
        result.data[idx++] = str1[i];
    }
    for (int i = 0; i < N2; ++i) {
        result.data[idx++] = str2[i];
    }

    //char data[] = "Hello";
    //fixed_string<6> result = { data, 6};
    return result;
}
...
void bar() {
    //char strData[] = "XXXXXXXXXXX";
    constexpr auto res = concat("AAAAA", " BBBBB");
    strPtr;
}

void foo() {
    //char strData[] = "Hello World";
    //strPtr = strData;
...
namespace v1 {
    constexpr std::basic_string_view<char> testStr = "Hello";

    template <typename T, const std::basic_string_view<T>&, typename>
    struct MyString;

    template <typename T, const std::basic_string_view<T>& S1, std::size_t... I1>
    struct MyString<T, S1, std::index_sequence<I1...>> {
        static constexpr const T value[]{ S1[I1]..., 0 };
    };
}
...
namespace v2 {
    ...
    template <typename T, std::size_t N, const char(&)[N], typename>
    struct MyString;

    template <typename T, std::size_t N, const char(&S1)[N], std::size_t... I1>
    struct MyString<T, N, S1, std::index_sequence<I1...>> {
        static constexpr const T value[]{ S1[I1]..., 0 };
    };
}

namespace v3 {
    constexpr auto testStr = std::ex::make_fixed_string("Hello");

    template <typename T, std::size_t N, const std::ex::basic_fixed_string<T, N>& Str, typename>
    struct MyString;

    template <typename T, std::size_t N, const std::ex::basic_fixed_string<T, N>& Str, std::size_t... I>
    struct MyString<T, N, Str, std::index_sequence<I...>> {
        static constexpr const T value[]{ Str[I]..., 0 };
    };
}


// auto - implicitly deduce size for result string
template <std::size_t MaxLenght = 9, typename CharT, std::size_t N>
constexpr auto PrettyName(std::ex::basic_fixed_string<CharT, N> str) {
    if (str.length() > MaxLenght) {
        return std::ex::make_fixed_string("XXX...YYY");
    }
    return str;
    //return str;
}
...
int main() {
    v1::MyString<char, v1::testStr, std::make_index_sequence<v1::testStr.size()>>::value;
    //v2::MyString<char, 6, "Hello", std::make_index_sequence<5>>::value;

    constexpr auto fxStr1 = std::ex::make_fixed_string("Hello");
    //static_assert(fxStr1.length() == 5, "");

    constexpr auto fxStr2 = std::ex::make_fixed_string("worl");
    //constexpr auto fxStr3 = fxStr1 + "..." + fxStr2;
    //static_assert(fxStr3 == "Hello...world", "");

    static constexpr auto v3testStr = PrettyName(fxStr1 + fxStr2);
    static_assert(v3testStr == "XXX...YYY", "");
    static_assert(v3testStr == "Helloworl", "");
    static_assert(v3testStr == "Hello...world", "");
